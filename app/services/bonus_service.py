...
class BonusService:
    ...
    def add_points(user_id, points, reason, max_per_day=None):
        """Начисляет баллы пользователю с проверкой лимита"""
        conn = get_db()
        
        if max_per_day:
            # Проверяем, сколько баллов уже начислено сегодня
    ...
    def claim_daily_bonus(user_id):
        """Ежедневный бонус с учётом стрейка"""
        conn = get_db()
        today = datetime.date.today()
        ...
        # Бонус увеличивается со стрейком
        bonus_amount = 10 + (streak // 7) * 5
        bonus_amount = min(bonus_amount, 50)  # Максимум 50 баллов
